...
These records are subordinate to official docs (`docs/architecture/`, `docs/execution/`). They do not create clinical agents, runtime actors, or authority surfaces.
...
Profile IDs map to Territory records under `.healthos-settler/territories/`. The Territory record holds the canonical domain definition (canonical docs, primary paths, invariants, validation commands). The Settler profile inherits from the Territory and adds profile-specific invariants, forbidden moves, and handoff requirements.
...
- Territory Schema: `.healthos-settler/territories/territory.schema.json`
