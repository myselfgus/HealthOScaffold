...
References Territory record: `.healthos-settler/territories/aaci.json`
...
1. `docs/architecture/09-aaci.md` — AACI runtime design, provider routing, and non-authorizing boundary
2. `docs/architecture/28-first-slice-executable-path.md` — first-slice executable path and session pipeline
3. `docs/architecture/20-runtime-operational-policy.md` — runtime operational policy for AACI and session management
4. `docs/execution/10-invariant-matrix.md` — invariants for AACI non-authority and Core mediation
5. `docs/execution/skills/aaci-skill.md` — AACI engineering skill reference
6. `docs/architecture/49-mental-space-runtime.md` — MSR pipeline and transcript normalization posture
...
- `swift/Sources/HealthOSAACI/` — AACI runtime Swift source
- `swift/Sources/HealthOSSessionRuntime/SessionRunner.swift` — canonical first-slice session orchestration
- `swift/Sources/HealthOSProviders/` — provider adapter implementations
- `docs/execution/todo/runtimes-and-aaci.md` — AACI/runtime domain TODO tracker
...
2. Updated TODO entry in `docs/execution/todo/runtimes-and-aaci.md` reflecting task status.
...
This Settler profile is an engineering instruction document. It is not a clinical agent, runtime actor, HealthOS Core actor, or authority record. It does not grant merge authority, clinical access, or production-readiness. It does not implement AACI runtime behavior or provider routing. It does not make AACI a clinical authority or allow it to resolve Core governance gates. Official docs (`docs/architecture/`, `docs/execution/`) remain canonical.
