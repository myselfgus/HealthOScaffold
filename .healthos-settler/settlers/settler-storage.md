...
References Territory record: `.healthos-settler/territories/storage-and-data.json`
...
1. `docs/architecture/05-data-layers.md` — data layer stack, lawfulContext, and Core-owned storage law
2. `docs/architecture/07-storage.md` — storage contracts and object integrity strategy
3. `docs/architecture/21-object-integrity-strategy.md` — object integrity and provenance chain
4. `docs/execution/10-invariant-matrix.md` — invariants for storage, lawfulContext, and reidentification
5. `docs/execution/skills/storage-data-layer-skill.md` — storage/data layer engineering skill reference
6. `docs/execution/skills/backup-restore-retention-export-skill.md` — backup, restore, retention, and export skill
...
- `swift/Sources/HealthOSCore/StorageContracts.swift` — Core-governed storage contracts
- `swift/Sources/HealthOSCore/ReidentificationGovernance.swift` — reidentification governance
- `sql/migrations/001_init.sql` — SQL schema baseline
- `schemas/contracts/` — JSON Schema storage contracts
- `docs/architecture/` — storage-related architecture docs (read only unless explicitly scoped)
- `docs/execution/todo/data-storage.md` — data/storage domain TODO tracker
...
2. Updated TODO entry in `docs/execution/todo/data-storage.md` reflecting task status.
