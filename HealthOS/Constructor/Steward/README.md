...
## Canonical truth
Official docs remain canonical (`README.md`, `AGENTS.md`, `CLAUDE.md`, `HealthOS/Shared/docs/execution/*`). Steward memory does not replace official docs.

## Current baseline contents

- session persistence: `HealthOS/Constructor/Steward/memory/sessions/`
- derived memory/index material: `HealthOS/Constructor/Steward/memory/`
- provider config scaffolds: `HealthOS/Constructor/Steward/providers/`
...
- `HealthOS/Constructor/Steward/memory/automations/daily-todo-tracker/` — daily TODO digests (`YYYY-MM-DD.md` + `latest.md`)
- `HealthOS/Constructor/Steward/memory/automations/sync-work-plan/memory.md` — work plan sync log (truth table + changes per run)
- `HealthOS/Constructor/Steward/memory/automations/update-claude-md/memory.md` — CLAUDE.md update log
...
Current `healthos-steward` CLI baseline includes 10 implemented deterministic top-level commands in `HealthOS/Constructor/ts/agent-infra/healthos-steward/`: `status`, `runtime`, `session`, `list` (`territories`, `settlers`, `settlements`), `inspect` (`territory <id>`, `settler <id>`, `settlement <id>`), `next`, `generate-prompt <settlement-id>`, `validate-settlement <settlement-id>`, `pr-draft <settlement-id>`, and `build-memory`. This directory remains derived state; do not infer additional deterministic operations from files here unless those operations are implemented in the package.

## Providers
- Base config scaffold: `HealthOS/Constructor/Steward/providers/providers.example.json`
- Local override (gitignored): `HealthOS/Constructor/Steward/providers/providers.local.json`
- Schema: `HealthOS/Constructor/Steward/providers/providers.schema.json`
...
## Logs
Runtime invocation logs are written to `HealthOS/Shared/runtime-data/steward/model-invocations.jsonl`.
