...
Build: `make ts-build` or `cd HealthOS/Constructor/ts && pnpm install && pnpm -r build`
...
cd HealthOS/Constructor/ts && npx --yes --workspace @healthos/steward healthos-steward generate-prompt <id>
cd HealthOS/Constructor/ts && npx --yes --workspace @healthos/steward healthos-steward validate-settlement <id>
cd HealthOS/Constructor/ts && npx --yes --workspace @healthos/steward healthos-steward pr-draft <id>
...
Forge MCP — stdio MCP server exposing 10 `steward_*` tools for deterministic repository maintenance. Used by Steward for Xcode, Xcode Intelligence, and external coding assistants. Not a clinical, runtime, or governance MCP server. See `HealthOS/Shared/docs/architecture/45-healthos-xcode-agent.md`.
