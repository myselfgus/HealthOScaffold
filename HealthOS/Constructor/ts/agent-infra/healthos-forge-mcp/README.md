...
| `steward_build_memory` | Builds 6 derived memory snapshots in `HealthOS/Constructor/Steward/memory/derived/` |
...
```json
{
  "mcpServers": {
    "healthos-forge": {
      "command": "node",
      "args": ["/Users/healthOS/HealthOScaffold/HealthOS/Constructor/ts/agent-infra/healthos-forge-mcp/dist/server.js"]
    }
  }
}
```

Replace the path with the absolute path to `dist/server.js` in your clone. After adding this config, `cd HealthOS/Constructor/ts && npm install` will ensure `dist/server.js` is present.

### b) Generic stdio MCP client

```
command: node /path/to/HealthOScaffold/HealthOS/Constructor/ts/agent-infra/healthos-forge-mcp/dist/server.js
```
...
Compiled output is excluded from version control (standard for TypeScript packages). Running `cd HealthOS/Constructor/ts && npm install` is sufficient to build `dist/` via the `prepare` lifecycle script.
