...
export function handleGetHandoff(): HandlerResult {
  try {
    ...
    return {
      content: JSON.stringify(
        { content, source: "HealthOS/Shared/docs/execution/12-next-agent-handoff.md", _non_canonical: NON_CANONICAL },
        null,
        2
      ),
    };
...
export function handleListSettlements(): HandlerResult {
  try {
    ...
    const active = settlements.active.map((s) => ({
      ...
      path: `HealthOS/Constructor/Steward/settlements/${s.statusDir}/${s.fileId}.md`,
    }));
...
export function handleGeneratePrompt(
  ...
  try {
    ...
    const outputDir = join(repoRoot, "HealthOS/Constructor/Steward", "prompts", "generated");
    const outputPath = join(outputDir, `${resolved.fileId}.md`);
    const relPath = `HealthOS/Constructor/Steward/prompts/generated/${resolved.fileId}.md`;
