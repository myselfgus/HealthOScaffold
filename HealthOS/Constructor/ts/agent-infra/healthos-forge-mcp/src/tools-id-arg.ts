...
export function registerIdArgTools(server: McpServer): void {
  ...
  server.registerTool(
    ...
    {
      description:
        "Generates a 16-section PromptSpec Markdown from a Settlement record, its Territory records, and Settler profiles. Writes output to HealthOS/Constructor/Steward/prompts/generated/<id>.md. No LLM calls. Repository-maintenance tool. No clinical authority.",
      inputSchema: {
        id: z
          .string()
          .min(1)
