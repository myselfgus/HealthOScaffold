...
function validateTerritoryIds(): ConstructionValidationCheck {
  const dir = join(repoRoot, "HealthOS/Constructor/Settler", "territories");
  const ids = readdirSync(dir)
    .filter((f) => f.endsWith(".json") && f !== "territory.schema.json")
    .map((f) => f.replace(/\.json$/, ""))
    .sort();
  const joined = ids.join(", ");
  const schemaText = readRel("HealthOS/Constructor/Settler/settlements/SCHEMA.md");
  const templateText = readRel("HealthOS/Constructor/Steward/settlements/templates/settlement-template.md");
...
function validateForgeTools(): ConstructionValidationCheck {
  const toolsText = readRel("HealthOS/Constructor/ts/agent-infra/healthos-forge-mcp/src/tools.ts");
  const idArgText = readRel("HealthOS/Constructor/ts/agent-infra/healthos-forge-mcp/src/tools-id-arg.ts");
...
function validateDerivedNonCanonical(): ConstructionValidationCheck {
  const dirs = [
    ...
    "HealthOS/Constructor/Steward/prompts/generated",
  ];
