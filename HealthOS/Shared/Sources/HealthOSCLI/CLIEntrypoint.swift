...
struct HealthOSCLI {
    static func main() async {
        do {
            ...
            print("gos_runtime_lifecycle=\(bridgeState.gosRuntimeState.lifecycle.rawValue)")
            print("gos_runtime_spec=\(bridgeState.gosRuntimeState.specId ?? "none")")
            print("gos_runtime_bundle=\(bridgeState.gosRuntimeState.bundleId ?? "none")")
            print("gos_runtime_workflow=\(bridgeState.gosRuntimeState.workflowTitle ?? "none")")
            print("gos_runtime_binding_source=\(bridgeState.gosRuntimeState.bindingPlanSource?.rawValue ?? "none")")
            print("gos_runtime_bound_actors=\(bridgeState.gosRuntimeState.mediationSummary?.boundActors.map(\.actorId).joined(separator: ",") ?? "none")")
            print("gos_runtime_draft_mediations=\(bridgeState.gosRuntimeState.mediationSummary?.draftMediations.map { $0.draftKind.rawValue + ":" + ($0.provenanceOperation ?? "none") }.joined(separator: ",") ?? "none")")
    ...
    private static func resolveRuntimeRoot(fileManager: FileManager = .default) throws -> URL {
        ...
        for _ in 0..<5 {
            if fileManager.fileExists(atPath: candidate.appending(path: "Shared/runtime-data").path) {
                return candidate.appending(path: "Shared/runtime-data/Users/Shared/HealthOS").standardizedFileURL
            }
            let healthOSRoot = candidate.appending(path: "HealthOS")
            if fileManager.fileExists(atPath: healthOSRoot.appending(path: "Shared/runtime-data").path) {
                return healthOSRoot.appending(path: "Shared/runtime-data/Users/Shared/HealthOS").standardizedFileURL
            }
            let parent = candidate.deletingLastPathComponent()
            guard parent.path != candidate.path else { break }
            candidate = parent
        }
        throw CLIError.invalidState("Could not resolve Shared/runtime-data root for GOS promotion command.")
    }
}
