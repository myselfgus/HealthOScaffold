...
Xcode Intelligence provides native workspace-aware engineering-agent capability aligned with the Apple sovereignty thesis in `HealthOS/Shared/docs/architecture/46-apple-sovereignty-architecture.md`:
...
Xcode Intelligence (Apple-controlled engineering runtime surface)
  ...
  │    (HealthOS/Constructor/Steward/memory/derived/)
...
Codex may act as an external executor for Steward-scoped Xcode-facing repository maintenance. Its bounded role is to review and propose PRs for repository-maintenance automation guidance, Xcode/Steward instructions, and automation drift. This does not create a new Steward category, does not make Codex an internal Steward provider, and does not grant merge or clinical/runtime authority.

The canonical model is defined in `HealthOS/Shared/docs/architecture/47-steward-settler-engineering-model.md`.
...
- skill files under `HealthOS/Shared/docs/execution/skills/`
...
- package: `HealthOS/Constructor/ts/agent-infra/healthos-forge-mcp/`
...
Location: `HealthOS/Constructor/Steward/memory/derived/`
...
Official docs (`HealthOS/Shared/docs/`, `README.md`, `CLAUDE.md`, `AGENTS.md`) remain canonical. Memory is a speed layer, not an authority layer.
...
- follow the agent operating protocol in `HealthOS/Shared/docs/execution/01-agent-operating-protocol.md`
...
Allowed:
- synchronize derived memory under `HealthOS/Constructor/Steward/memory/derived/` against official docs
...
- official docs precedence: `HealthOS/Shared/docs/`, `README.md`, `CLAUDE.md`, `AGENTS.md` over memory
...
`HealthOS/Shared/docs/architecture/44-project-steward-agent.md` is the historical reference for the Steward scaffold before this architectural realignment. It is preserved as reasoning history.
...
- `HealthOS/Constructor/Steward/memory/derived/` as derived memory location
...
| Derived repository memory | scaffolded contract (HealthOS/Constructor/Steward/memory/ exists) | Official docs remain canonical source |
...
Maturity levels used: doctrine-only, scaffolded contract, implemented seam, tested operational path, production-hardened (per canonical ladder in `HealthOS/Shared/docs/execution/README.md`). Inv 43 applies: scaffold or foundation phase closure is not production readiness.
