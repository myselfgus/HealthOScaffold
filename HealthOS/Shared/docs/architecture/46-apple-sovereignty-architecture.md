...
**GAP-003 reframe.** SQL/object backends are complementary query/index/projection substrates, not parity replacements for file-backed canonical storage. Future SQL/object adapters must preserve `lawfulContext` and layer semantics. See `HealthOS/Shared/docs/execution/14-final-gap-register.md`.

**Steward for Xcode posture.** Doc 45 (`HealthOS/Shared/docs/architecture/45-healthos-xcode-agent.md`) defines Steward for Xcode. Steward for Xcode integrates with Xcode Intelligence as an Apple-controlled engineering runtime surface, while HealthOS contributes instructions, HealthOS Forge MCP (`healthos-forge-mcp`), derived repository memory, and deterministic CLI operations. This follows directly from the Apple-first engineering posture: Apple's native workspace intelligence belongs to the same sovereign substrate family. HealthOS does not duplicate what Xcode provides natively.
...
- `HealthOS/Shared/docs/architecture/01-overview.md` — canonical hierarchy and HealthOS constitutional placement
- `HealthOS/Shared/docs/architecture/05-data-layers.md` — storage layer definitions and sensitivity taxonomy
- `HealthOS/Shared/docs/architecture/07-storage-and-sql.md` — storage architecture and SQL/file relationship
- `HealthOS/Shared/docs/architecture/45-healthos-xcode-agent.md` — Steward for Xcode: Xcode Intelligence as native runtime surface for Steward, following from Apple-first engineering posture
- `HealthOS/Shared/docs/execution/10-invariant-matrix.md` — Inv 14 (lawfulContext enforcement), Inv 15 (storage layer sensitivity), Inv 17/22 (provider honesty and anti-fake posture)
- `HealthOS/Shared/docs/execution/14-final-gap-register.md` — GAP-003: SQL/object complementary backend, reframed per this document
