...
The work unit also generated an editable HealthOS visual overview PPTX outside the commit because the repository does not yet have a clear `HealthOS/Shared/docs/assets/presentations/` versioning pattern. Future documentation work can decide whether to create a durable versioned asset path such as `HealthOS/Shared/docs/assets/presentations/healthos-visual-overview.pptx`; do not link to a non-versioned deck from canonical docs.

## Construction-system track

ST-010 started the construction-system track by adding `HealthOS/Shared/docs/execution/22-steward-construction-operating-model.md` and the initial `HealthOS/Constructor/Settler/` / `HealthOS/Constructor/Steward/` construction skeletons.

ST-011 created the Territory Registry under `HealthOS/Constructor/Settler/territories/`, including `territory.schema.json` and initial Territory records for repository domains. Territory records are construction metadata only and remain subordinate to official docs.

ST-011B created the product technical specification baseline at `HealthOS/Shared/docs/product/01-healthos-technical-product-specification.md`. Future construction and product tasks should read this product baseline together with canonical architecture and execution docs when generating scoped work units.
...
1. Read `HealthOS/Shared/docs/execution/21-structural-ontology-and-product-readiness-plan.md` → Priority Grid.
...
6. Load the matching skill from `HealthOS/Shared/docs/execution/skills/` if one exists.
...
- Swift Package: `HealthOS/Package.swift` targets macOS 26.0, Swift tools 6.2. Canonical Apple build graph.
- Xcode workspace: `HealthOS.xcworkspace` → points to `HealthOS/Package.swift`. Not a replacement for TS/Python tooling.
...
- `HealthOS/Constructor/ts/packages/` conflates PRODUCT, BUILD, and AGENT packages. STR-003 separates them.
...
`HealthOS/Shared/docs/architecture/48-native-macos-ui-design-system-and-app-shells.md` defines scaffold scope for macOS 26+ Liquid Glass app-shell work. Use `HealthOS/Shared/docs/execution/skills/native-macos-ui/SKILL.md` for UI work. Do not move Core law into SwiftUI.

## MSR note

`HealthOS/Shared/docs/architecture/49-mental-space-runtime.md` and `HealthOS/Shared/docs/execution/skills/mental-space-runtime-skill.md` are the canonical refs. `MSR` is now the official sigla for Mental Space Runtime, and transcript normalization belongs to `HealthOSSessionRuntime`, not to MSR. Keep outputs derived/gated/app-safe/non-authorizing at all times.
...
- `HealthOS/Constructor/Settler/` holds Territory Registry and Settler profile records.
- `HealthOS/Constructor/Steward/` holds derived memory, prompts, Settlement records, policies, and Managed Agent state.
- `HealthOS/Constructor/Territory/` is a prior documentation-only root for historical Territory scaffold references.
