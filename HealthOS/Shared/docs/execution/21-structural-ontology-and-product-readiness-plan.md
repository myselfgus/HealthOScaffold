...
6. Update `HealthOS/Shared/docs/execution/02-status-and-tracking.md` and mark the task `DONE` in this document and in the relevant TODO file.
...
| STR-002 | **P1** | DONE | Archive `Skill macOS/` to `HealthOS/Shared/docs/reference/mental-space-legacy/` | — |
| STR-003 | **P1** | DONE | Separate AGENT packages from PRODUCT in `HealthOS/Constructor/ts/packages/` | — |
...
| Stage readiness — Custom | CloudClinic Custom — incomplete / needs-review | CloudClinic has contracts and a placeholder executable, but the Custom is missing degraded behavior and exact upstream-surface commitments | Complete the CloudClinic Custom in `HealthOS/Shared/docs/architecture/50-app-layer-boundary-and-reference-apps.md` or a follow-up Custom doc |
...
| Legacy / needs-review | `HealthOS/Shared/docs/execution/18-healthos-xcode-agent-task-tracker.md` streams after XA-002 | Historical Xcode-agent tracker overlaps later Steward/Forge MCP construction work | Do not select without reconciling against doc 22, doc 19, and current implemented Steward/Forge MCP baseline |
...
- `HealthOS/Package.swift` — add `"HealthOSProviders"` to `HealthOSMSR` dependencies array
- `HealthOS/Shared/docs/execution/02-status-and-tracking.md` — add STR-001 completion entry
- `HealthOS/Shared/docs/execution/21-structural-ontology-and-product-readiness-plan.md` — mark STR-001 DONE
...
cd HealthOS && swift package dump-package | grep -A5 HealthOSMSR
make validate-all
```
...
**Why:** ASL (Analise Sistemica da Linguagem) is the first stage of the clinical pipeline. It extracts 8 psycholinguistic domains from a transcript. The prompt is validated against 400 patients and lives at `HealthOS/Tier2-GOS-Runtimes/Sources/HealthOSMSR/Prompts/asl-system.md`. The executor stub always throws `.providerUnavailable`. This task makes it callable.
...
- `HealthOS/Tier2-GOS-Runtimes/Sources/HealthOSMSR/Executors/ASLExecutor.swift` — implement real executor
- `HealthOS/Tier2-GOS-Runtimes/Sources/HealthOSMSR/MSRPipeline.swift` — update orchestrator to call ASL executor
- `HealthOS/Shared/Tests/HealthOSTests/MentalSpaceRuntimeTests.swift` — add ASL executor tests
- `HealthOS/Shared/docs/execution/02-status-and-tracking.md` — completion entry
- `HealthOS/Shared/docs/execution/todo/runtimes-and-aaci.md` — mark RT-MSR-001 DONE
- `HealthOS/Shared/docs/execution/21-structural-ontology-and-product-readiness-plan.md` — mark RT-MSR-001 DONE
...
**Why:** `Skill macOS/` at repository root implies it is an active production component at the same level as `swift/`, `HealthOS/Constructor/ts/`, and `HealthOS/Tier1-Mestral-Core/Schemas/`. It is not. The prompt contracts are already migrated to `HealthOS/Tier2-GOS-Runtimes/Sources/HealthOSMSR/Prompts/`. The TS scripts are reference implementations during the Swift migration, nothing more.

**Files to touch:**
```bash
# git mv preserves full history
git mv "Skill macOS" "HealthOS/Shared/docs/reference/mental-space-legacy"
```
- `HealthOS/Shared/docs/reference/mental-space-legacy/README.md` — new file, marks these as archived reference
- `HealthOS/Shared/docs/architecture/49-mental-space-runtime.md` — update any reference to `Skill macOS/` → `HealthOS/Shared/docs/reference/mental-space-legacy/`
- `HealthOS/Shared/docs/execution/02-status-and-tracking.md` — completion entry
- `HealthOS/Shared/docs/execution/21-structural-ontology-and-product-readiness-plan.md` — mark STR-002 DONE

**`HealthOS/Shared/docs/reference/mental-space-legacy/README.md` content required:**
```
...
version-controlled at:
  HealthOS/Tier2-GOS-Runtimes/Sources/HealthOSMSR/Prompts/
...
- `ls HealthOS/Shared/docs/reference/mental-space-legacy/` → lists all 8 original files + README.md
- `git log --oneline HealthOS/Shared/docs/reference/mental-space-legacy/4-asl.ts` → shows history
- `make validate-docs` PASS
- `make validate-all` PASS

---

### STR-003: Separate AGENT packages from PRODUCT in `HealthOS/Constructor/ts/packages/`

**Priority:** P1 | **Status:** DONE (2026-04-29) | **Branch:** `feat/str-003-ts-agent-infra-dir`

**Why:** `HealthOS/Constructor/ts/packages/` currently conflates three distinct layers — product runtimes, BUILD tooling, and AGENT infrastructure — at the same directory level. This creates false structural equivalence between components that must not be mixed operationally.
...
```
HealthOS/Constructor/ts/packages/             # PRODUCT + BUILD only
├── contracts/
...
HealthOS/Constructor/ts/agent-infra/          # AGENT only
├── healthos-steward/
└── mcp-local/
```
...
**Files to touch:**
```bash
mkdir -p HealthOS/Constructor/ts/agent-infra
git mv HealthOS/Constructor/ts/packages/healthos-steward HealthOS/Constructor/ts/agent-infra/healthos-steward
git mv HealthOS/Constructor/ts/packages/mcp-local HealthOS/Constructor/ts/agent-infra/mcp-local
```
- `HealthOS/Constructor/ts/package.json` and `HealthOS/Constructor/ts/package-lock.json` — include `"agent-infra/*"` in npm workspaces while preserving `"packages/*"`
- `CLAUDE.md` — update Steward CLI path references from `HealthOS/Constructor/ts/agent-infra/healthos-steward` → `HealthOS/Constructor/ts/agent-infra/healthos-steward`
- `README.md` — same path updates
- `HealthOS/Shared/docs/architecture/45-healthos-xcode-agent.md` — same path updates
- `HealthOS/Shared/docs/execution/17-healthos-xcode-agent-migration-plan.md` — same path updates
- `HealthOS/Shared/docs/execution/02-status-and-tracking.md` — completion entry
- `HealthOS/Shared/docs/execution/21-structural-ontology-and-product-readiness-plan.md` — mark STR-003 DONE

**Definition of done:**
- `ls HealthOS/Constructor/ts/agent-infra/healthos-steward` → no such directory
- `ls HealthOS/Constructor/ts/agent-infra/healthos-steward/src/` → source files present with history
- `cd HealthOS/Constructor/ts && npm run build` PASS (pnpm workspace resolves both paths)
- `make ts-build` PASS
- `make validate-docs` PASS (no broken path references)

**Validation commands:**
```bash
cd HealthOS/Constructor/ts && npm run build
make ts-build
make validate-docs
make validate-all
```
...
**Files to touch:**
```bash
git mv swift/Sources/HealthOSFirstSliceSupport HealthOS/Tier2-GOS-Runtimes/Sources/HealthOSSessionRuntime
```
- `HealthOS/Package.swift` — rename all occurrences of `HealthOSFirstSliceSupport` → `HealthOSSessionRuntime`
- `HealthOS/Tier2-GOS-Runtimes/Sources/HealthOSSessionRuntime/SessionRunner.swift` — rename file to `SessionRunner.swift` and update any internal references to "FirstSlice" in public API names (keep internal names if needed for now; public API only)
- `HealthOS/Tier2-GOS-Runtimes/Sources/HealthOSSessionRuntime/ScribeSessionAdapter.swift` — rename to `ScribeSessionAdapter.swift` and update public type names
- `HealthOS/Shared/Sources/HealthOSCLI/CLIEntrypoint.swift` — update imports: `HealthOSFirstSliceSupport` → `HealthOSSessionRuntime`
- `HealthOS/Tier4-Stages-Cast/Scribe/Sources/Scribe/` — update imports
- `HealthOS/Shared/Tests/HealthOSTests/` — update imports and any references in test files
- `HealthOS/Shared/docs/execution/02-status-and-tracking.md` — completion entry
- `HealthOS/Shared/docs/execution/21-structural-ontology-and-product-readiness-plan.md` — mark STR-004 DONE
- Update any doc references to `HealthOSSessionRuntime` in `HealthOS/Shared/docs/architecture/`
...
struct VeridiaEntrypoint {
    static func main() {
        ...
        print("HealthOSVeridia: scaffold placeholder — no final UI shell (see HealthOS/Shared/docs/architecture/12-veridia.md)")
    }
}
```
...
**`HealthOS/Package.swift` additions:**
```swift
.executableTarget(
    name: "Veridia",
...
- `HealthOS/Tier4-Stages-Cast/Veridia/Sources/Veridia/VeridiaEntrypoint.swift` (new)
- `HealthOS/Tier4-Stages-Cast/CloudClinic/Sources/CloudClinic/CloudClinicEntrypoint.swift` (new)
- `HealthOS/Shared/docs/execution/02-status-and-tracking.md`
- `HealthOS/Shared/docs/execution/21-structural-ontology-and-product-readiness-plan.md`

**Definition of done:**
- `swift build` PASS — CLI, Scribe, Veridia, CloudClinic, and test targets build
- `swift run Veridia --smoke-test` exits 0 and prints scaffold message
- `swift run CloudClinic --smoke-test` exits 0 and prints scaffold message
- `swift test` PASS — no regressions
- `make validate-all` PASS
...
**Definition of done:**
- `Veridia` can run a governed user-agent session scaffold (seeded input, governed output, honest degraded state)
...
**Files to touch:**
- `HealthOS/Tier2-GOS-Runtimes/Sources/HealthOSProviders/AppleFoundationModelsAdapter.swift` (new or update existing stub)
- `HealthOS/Tier2-GOS-Runtimes/Sources/HealthOSAACI/AACI.swift` — update normalization provider selection to use real adapter when available
- `HealthOS/Shared/Tests/HealthOSTests/MentalSpaceRuntimeTests.swift` — add normalization with real local provider test (mark as requiring macOS 26 device)
- `HealthOS/Shared/docs/execution/02-status-and-tracking.md`
- `HealthOS/Shared/docs/execution/21-structural-ontology-and-product-readiness-plan.md`
...
**Completion note (2026-04-30):** RT-PROVIDER-001 completed by adding `AppleFoundationModelsAdapter.swift` and moving `AppleFoundationProvider` from an always-stubbed provider to a real local Foundation Models adapter. The implementation uses the current SDK shape: `SystemLanguageModel.default.availability`, locale support checks, `LanguageModelSession(model:tools:instructions:)`, and `respond(to:options:)`. Normalization uses the real provider only when Foundation Models is compiled in, the current locale is supported, and `SystemLanguageModel.default` is available. If the framework is unavailable, the device is not eligible, Apple Intelligence is disabled, the model is not ready, the locale is unsupported, or tests force stub mode, the path remains explicit degraded/stub-only and does not persist stub text as a normalized transcript. Remote fallback remains denied for v1. No ASL, VDLP, or GEM behavior was expanded. Validation: `cd HealthOS && swift build` PASS; `cd HealthOS && swift test` PASS (260 tests, 0 failures; real-provider availability test executed on this machine).

---

### RT-ASYNC-001: SQL-backed async runtime executor

**Priority:** P3 | **Status:** READY (Tier 1 platform foundation) | **Branch:** `feat/rt-async-001-sql-executor`

**Why:** The async runtime is currently `InMemoryAsyncJobRuntime` only. The SQL schema exists (`HealthOS/Tier1-Mestral-Core/SQL/migrations/001_init.sql` includes `async_jobs`, `async_job_attempts`, `async_job_events` tables). This task wires a real PostgreSQL-backed executor using those tables.

**Prerequisite:** Local PostgreSQL running per single-node runbook. Connection infrastructure in `HealthOSCore` or `HealthOSProviders` must be completed or explicitly environment-gated in this task.

**Files to touch:**
- `HealthOS/Tier1-Mestral-Core/Sources/HealthOSCore/AsyncRuntimeJobs.swift` — add `PostgreSQLAsyncJobRuntime` conforming to same protocol as `InMemoryAsyncJobRuntime`
- `HealthOS/Shared/Tests/HealthOSTests/AsyncRuntimeGovernanceTests.swift` — add SQL-backed executor tests (using a local test DB)
- `HealthOS/Tier1-Mestral-Core/SQL/migrations/001_init.sql` — no changes required (schema already exists)
- `HealthOS/Shared/docs/execution/02-status-and-tracking.md`
- `HealthOS/Shared/docs/execution/21-structural-ontology-and-product-readiness-plan.md`
...
**Priority:** P3 | **Status:** READY (Tier 1 platform/runtime foundation) | **Branch:** `feat/rt-retrieval-001-semantic-retrieval`

**Why:** Retrieval currently returns honest `unavailable` for semantic queries. The governed retrieval contracts and fail-closed embedding boundary exist. This task wires a real embeddings provider (Apple or remote, per policy) or explicitly policy-approved local path to make semantic retrieval operational without fake semantic scores.
...
**Files to touch:**
- `HealthOS/Tier2-GOS-Runtimes/Sources/HealthOSProviders/EmbeddingsAdapter.swift` (new)
- `HealthOS/Tier1-Mestral-Core/Sources/HealthOSCore/RetrievalMemoryGovernance.swift` — update semantic retrieval to use adapter when available
- `HealthOS/Shared/Tests/HealthOSTests/RetrievalMemoryGovernanceTests.swift` — add semantic retrieval tests
- `HealthOS/Shared/docs/execution/02-status-and-tracking.md`
- `HealthOS/Shared/docs/execution/21-structural-ontology-and-product-readiness-plan.md`
...
2. Add a completion entry to `HealthOS/Shared/docs/execution/02-status-and-tracking.md` (follow existing format)
3. Update the relevant `HealthOS/Shared/docs/execution/todo/*.md` file
4. Update `HealthOS/Shared/docs/execution/11-current-maturity-map.md` if the task advances a component's maturity tier
...
**Completion note (2026-04-29):** STR-003 completed by moving `@healthos/steward` and `@healthos/mcp-local` from `HealthOS/Constructor/ts/packages/` to `HealthOS/Constructor/ts/agent-infra/` with history preserved via `git mv`; npm workspace updated to include `agent-infra/*`; docs path references updated. Known TS18003 blocker for steward was resolved by restoring minimal `src/` entrypoints to satisfy the package build contract honestly. No HealthOS clinical/runtime behavior was changed.
