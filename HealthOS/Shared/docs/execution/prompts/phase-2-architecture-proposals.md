...
7. **Tracking is mandatory.** After each task, update `HealthOS/Shared/docs/execution/02-status-and-tracking.md` and the relevant `HealthOS/Shared/docs/execution/todo/*.md` in the same work unit.
...
```
1.  HealthOS/Shared/docs/execution/20-documental-todos-work-plan.md           ← tasks 4–6 in the master plan
2.  HealthOS/Shared/docs/execution/01-agent-operating-protocol.md             ← operating rules
3.  HealthOS/Shared/docs/execution/10-invariant-matrix.md                     ← all 43 invariants
4.  HealthOS/Shared/docs/execution/02-status-and-tracking.md                  ← current status (top 80 lines)
5.  HealthOS/Shared/docs/execution/todo/core-laws.md                          ← CL-006 definition and context
6.  HealthOS/Shared/docs/execution/todo/ops-network-ml.md                     ← OPS-003 definition and context
7.  HealthOS/Shared/docs/execution/19-settler-model-task-tracker.md           ← ST-004 definition
8.  HealthOS/Shared/docs/architecture/06-core-services.md                     ← CL-006 target file (READ IN FULL)
9.  HealthOS/Shared/docs/adr/0006-local-swift-ts-seam.md                     ← CL-003 seam decision (loopback HTTP)
10. HealthOS/Shared/docs/architecture/14-operations-runbook.md                ← OPS-003 target file (READ IN FULL)
11. HealthOS/Shared/docs/architecture/26-operator-observability-contract.md   ← OPS-002 visibility contract
12. HealthOS/Shared/docs/architecture/47-steward-settler-engineering-model.md ← ST-004 canonical model
13. HealthOS/Shared/docs/execution/17-healthos-xcode-agent-migration-plan.md  ← WS-2 definition
14. CLAUDE.md                                                  ← healthos-mcp boundary doctrine
```

After reading, confirm:
- `HealthOS/Shared/docs/architecture/06-core-services.md` exists → YES/NO
- `HealthOS/Shared/docs/architecture/14-operations-runbook.md` exists → YES/NO
- `HealthOS/Shared/docs/architecture/26-operator-observability-contract.md` exists → YES/NO
- `HealthOS/Shared/docs/architecture/47-steward-settler-engineering-model.md` exists → YES/NO
- `HealthOS/Shared/docs/adr/0006-local-swift-ts-seam.md` exists → YES/NO
...
**Source**: `HealthOS/Shared/docs/execution/todo/core-laws.md` → CL-006
**Target file**: `HealthOS/Shared/docs/architecture/06-core-services.md` (extend — do not replace)
**Optional target**: `HealthOS/Tier1-Mestral-Core/Schemas/contracts/local-service-error-envelope.schema.json` (create only if spec is complete enough)
**Priority**: Medium
...
- CL-003 fixed the local seam as: loopback HTTP + PostgreSQL metadata + filesystem/object references (from `HealthOS/Shared/docs/adr/0006-local-swift-ts-seam.md`)
- CL-004 established that deny is a governed refusal (not a crash) and failure is a runtime error (not a deny) — from the existing content of `HealthOS/Shared/docs/architecture/06-core-services.md`
...
### What to add to `HealthOS/Shared/docs/architecture/06-core-services.md`
...
If the envelope shape above is sufficiently precise, also create `HealthOS/Tier1-Mestral-Core/Schemas/contracts/local-service-error-envelope.schema.json` with a JSON Schema definition of the envelope. Include the `code` enum values for denied and error. Mark it as a proposal, not a final contract.
...
Update `HealthOS/Shared/docs/execution/todo/core-laws.md`:
- Move CL-006 from READY to COMPLETED.
- Add outcome block:
  ```
  ### CL-006 Add shared error-envelope proposal for local service boundaries
  Outcome:
  - added ## Shared error envelope section to HealthOS/Shared/docs/architecture/06-core-services.md
  ...
  - HealthOS/Tier1-Mestral-Core/Schemas/contracts/local-service-error-envelope.schema.json (if created)
  ```

Add entry to `HealthOS/Shared/docs/execution/02-status-and-tracking.md`.
...
**Source**: `HealthOS/Shared/docs/execution/todo/ops-network-ml.md` → OPS-003
**Target file**: `HealthOS/Shared/docs/architecture/14-operations-runbook.md` (extend — do not replace)
**Priority**: High
...
- OPS-002 defined minimum operator visibility indicators and alert classes — read `HealthOS/Shared/docs/architecture/26-operator-observability-contract.md` in full before writing; the incident detection signals must match the alert classes already defined
...
### What to add to `HealthOS/Shared/docs/architecture/14-operations-runbook.md`
...
- Error rate exceeds threshold defined in `HealthOS/Shared/docs/architecture/26-operator-observability-contract.md`
...
Update `HealthOS/Shared/docs/execution/todo/ops-network-ml.md`:
...
**Source**: `HealthOS/Shared/docs/execution/19-settler-model-task-tracker.md` → ST-004
**Target file**: `HealthOS/Shared/docs/architecture/47-steward-settler-engineering-model.md` (add new section)
**Supporting update**: `HealthOS/Shared/docs/execution/19-settler-model-task-tracker.md` (mark ST-004 done)
**Priority**: Medium
...
- The canonical model doc (`HealthOS/Shared/docs/architecture/47-steward-settler-engineering-model.md`) already defines `healthos-mcp` at a high level. You are adding a detailed operations specification section.
- The migration plan (`HealthOS/Shared/docs/execution/17-healthos-xcode-agent-migration-plan.md` WS-2) defines the implementation objective. The spec must be consistent with that objective.

### What to add to `HealthOS/Shared/docs/architecture/47-steward-settler-engineering-model.md`
...
**`scan-status`**
- Reads `HealthOS/Shared/docs/execution/02-status-and-tracking.md` and returns current phase and recent completions
- Input: `{}` (no parameters)
...
**`next-task`**
- Reads all `HealthOS/Shared/docs/execution/todo/*.md` files and returns the highest-priority READY task
...
**`read-gap-register`**
- Reads `HealthOS/Shared/docs/execution/14-final-gap-register.md` and returns open gaps
- Input: `{ "filter": "open" | "all" }` (default: `open`)
- Output: `{ "gaps": [{ "id": string, "title": string, "severity": string, "status": string }] }`

**`get-handoff`**
- Reads `HealthOS/Shared/docs/execution/12-next-agent-handoff.md` and returns current handoff state
- Input: `{}`
- Output: `{ "current_state": string, "priority_gaps": string[], "next_tasks": string[], "validation_baseline": string[] }`

**`check-invariants`**
- Reads `HealthOS/Shared/docs/execution/10-invariant-matrix.md` and returns invariants flagged as having known gaps
...
Add an entry to `HealthOS/Shared/docs/execution/02-status-and-tracking.md` at the top of "Completed recently":
...
HealthOS/Shared/docs/execution/20-documental-todos-work-plan.md Phase 2.

Files touched:
- HealthOS/Shared/docs/architecture/06-core-services.md (CL-006 — shared error-envelope section added)
- HealthOS/Tier1-Mestral-Core/Schemas/contracts/local-service-error-envelope.schema.json (if created)
- HealthOS/Shared/docs/architecture/14-operations-runbook.md (OPS-003 — incident-response command vocabulary added)
- HealthOS/Shared/docs/architecture/47-steward-settler-engineering-model.md (ST-004 — operations spec section added)
...
```bash
# After Task 4 (CL-006):
git add HealthOS/Shared/docs/architecture/06-core-services.md HealthOS/Shared/docs/execution/todo/core-laws.md
...
git add HealthOS/Shared/docs/architecture/14-operations-runbook.md HealthOS/Shared/docs/execution/todo/ops-network-ml.md
...
git add HealthOS/Shared/docs/architecture/47-steward-settler-engineering-model.md \
        HealthOS/Shared/docs/execution/19-settler-model-task-tracker.md \
        HealthOS/Shared/docs/execution/02-status-and-tracking.md
...
gh pr create \
  ...
  --body "## Summary
- CL-006: shared error-envelope proposal added to HealthOS/Shared/docs/architecture/06-core-services.md
- OPS-003: incident-response command vocabulary added to HealthOS/Shared/docs/architecture/14-operations-runbook.md
- ST-004: healthos-mcp operations specification added to HealthOS/Shared/docs/architecture/47-steward-settler-engineering-model.md
...
- [ ] CL-006 moved to COMPLETED in HealthOS/Shared/docs/execution/todo/core-laws.md
- [ ] OPS-003 moved to COMPLETED in HealthOS/Shared/docs/execution/todo/ops-network-ml.md
- [ ] ST-004 moved to DONE in HealthOS/Shared/docs/execution/19-settler-model-task-tracker.md
...
- [ ] `HealthOS/Shared/docs/architecture/06-core-services.md` contains `## Shared error envelope for local service boundaries` with: rationale, three outcome classes, envelope shape, HTTP transport contract, `denied` code vocabulary (≥6 codes), `error` code vocabulary (≥2 codes), 6 invariants, maturity note
- [ ] `HealthOS/Shared/docs/architecture/14-operations-runbook.md` contains `## Incident-response command vocabulary` with all 4 incident categories and post-incident discipline section
- [ ] `HealthOS/Shared/docs/architecture/47-steward-settler-engineering-model.md` contains `## healthos-mcp operations specification` with boundary declaration, all 9 operations fully specified, shared constraints, maturity declaration
- [ ] CL-006 moved to COMPLETED in `HealthOS/Shared/docs/execution/todo/core-laws.md`
- [ ] OPS-003 moved to COMPLETED in `HealthOS/Shared/docs/execution/todo/ops-network-ml.md`
- [ ] ST-004 moved to DONE in `HealthOS/Shared/docs/execution/19-settler-model-task-tracker.md`
- [ ] `HealthOS/Shared/docs/execution/02-status-and-tracking.md` has PHASE-2-ARCHITECTURE-PROPOSALS entry
- [ ] `make validate-docs` passes
