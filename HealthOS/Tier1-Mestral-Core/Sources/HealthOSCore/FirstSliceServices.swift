...
public enum FirstSliceInvariantEnforcer {
    public static func ensureSOAPDraftCanFinalize(
        ...
        let logger = HealthOSTelemetry.logger(.validationGates)
        let signposter = HealthOSTelemetry.signposter(.validationGates)
        let state = signposter.beginInterval("gate.finalization.validate")
        defer { signposter.endInterval("gate.finalization.validate", state) }

        logger.debug("Validating finalization gate status=\(gate.resolution.resolution.rawValue, privacy: .public)")
        ...
        logger.info("Finalization gate accepted")
    }
}
...
public actor SimpleHabilitationService {
    ...
    public func validate(professional: Usuario, service: Servico) throws -> HabilitationContext {
        let logger = HealthOSTelemetry.logger(.coreValidation)
        let signposter = HealthOSTelemetry.signposter(.coreValidation)
        let state = signposter.beginInterval("habilitation.validate")
        defer { signposter.endInterval("habilitation.validate", state) }

        logger.debug("Core habilitation validation requested")
        ...
        logger.info("Core habilitation validation passed")
...
public actor SimpleConsentService {
    ...
    public func validate(patient: Usuario, finalidade: String) throws -> ConsentContext {
        let logger = HealthOSTelemetry.logger(.coreValidation)
        let signposter = HealthOSTelemetry.signposter(.coreValidation)
        let state = signposter.beginInterval("consent.validate")
        defer { signposter.endInterval("consent.validate", state) }

        logger.debug("Core consent validation requested")
        ...
        logger.info("Core consent validation passed")
...
public actor SimpleGateService {
    ...
    public func createRequest(for draft: ArtifactDraft) -> GateRequest {
        let logger = HealthOSTelemetry.logger(.validationGates)
        let signposter = HealthOSTelemetry.signposter(.validationGates)
        let state = signposter.beginInterval("gate.request.create")
        defer { signposter.endInterval("gate.request.create", state) }

        logger.info("Creating professional gate request for draft status=\(draft.status.rawValue, privacy: .public)")
        return GateRequest(
            draftId: draft.id,
    ...
    public func resolve(_ request: GateRequest, resolverUserId: UUID, approve: Bool) -> GateResolution {
        let logger = HealthOSTelemetry.logger(.validationGates)
        let signposter = HealthOSTelemetry.signposter(.validationGates)
        let state = signposter.beginInterval("gate.request.resolve")
        defer { signposter.endInterval("gate.request.resolve", state) }

        logger.info("Resolving professional gate approve=\(approve, privacy: .public)")
