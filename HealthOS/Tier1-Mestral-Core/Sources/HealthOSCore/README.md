...
graph TD
    ...
    JSONSCHEMA[HealthOS/Tier1-Mestral-Core/Schemas/\nJSON Schema contracts]:::extern
    TS[HealthOS/Constructor/ts/packages/contracts\nTypeScript contracts]:::extern
...
Core law is enforced through the invariant matrix (`HealthOS/Shared/docs/execution/10-invariant-matrix.md`). Key invariants guarded in this module:
...
- When adding new contract types, mirror the change to `HealthOS/Tier1-Mestral-Core/Schemas/` and `HealthOS/Constructor/ts/packages/contracts/` in the same work unit.
