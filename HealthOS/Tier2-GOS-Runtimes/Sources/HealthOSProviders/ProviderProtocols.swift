...
public actor ProviderRouter {
    ...
    public func register(_ provider: any LanguageModelProvider) throws {
        ...
        logRegistration(profile: provider.capabilityProfile)
    }
    ...
    public func routeLanguage(request: ProviderRoutingRequest) -> ProviderRoutingDecision {
        let signposter = HealthOSTelemetry.signposter(.providers)
        let interval = signposter.beginInterval("provider.route.language")
        let decision = route(request: request, providers: Array(languageProviders.values).sorted(by: { $0.providerName < $1.providerName })) {
            ProviderSelection(
        ...
        signposter.endInterval("provider.route.language", interval)
        logRoutingDecision(decision, request: request)
        return decision
    }

    public func routeSpeech(request: ProviderRoutingRequest) -> ProviderRoutingDecision {
        let signposter = HealthOSTelemetry.signposter(.providers)
        let interval = signposter.beginInterval("provider.route.speech")
        let decision = route(request: request, providers: Array(speechProviders.values).sorted(by: { $0.providerName < $1.providerName })) {
            ProviderSelection(
                providerId: $0.providerName,
                providerKind: $0.capabilityProfile.providerKind,
                taskClass: request.taskClass,
        ...
        signposter.endInterval("provider.route.speech", interval)
        logRoutingDecision(decision, request: request)
        return decision
    }
    ...
    public func routeEmbedding(request: ProviderRoutingRequest) -> ProviderRoutingDecision {
        let signposter = HealthOSTelemetry.signposter(.providers)
        let interval = signposter.beginInterval("provider.route.embedding")
        let decision = route(request: request, providers: Array(embeddingProviders.values).sorted(by: { $0.providerName < $1.providerName })) {
            ProviderSelection(
                providerId: $0.providerName,
                providerKind: $0.capabilityProfile.providerKind,
                taskClass: request.taskClass,
        ...
        signposter.endInterval("provider.route.embedding", interval)
        logRoutingDecision(decision, request: request)
        return decision
    }
    ...
    private func logRegistration(profile: ProviderCapabilityProfile) {
        HealthOSTelemetry.logger(.providers).info(
            "Provider registered kind=\(profile.providerKind.rawValue, privacy: .public) taskCount=\(profile.supportedTaskClasses.count, privacy: .public) stub=\(profile.isStub, privacy: .public)"
        )
    }

    private func logRoutingDecision(_ decision: ProviderRoutingDecision, request: ProviderRoutingRequest) {
        let status: String
        switch decision {
        case .selected:
            status = "selected"
        ...
        HealthOSTelemetry.logger(.providers).info(
            "Provider route task=\(request.taskClass.rawValue, privacy: .public) layer=\(request.dataLayer.rawValue, privacy: .public) status=\(status, privacy: .public)"
        )
    }
