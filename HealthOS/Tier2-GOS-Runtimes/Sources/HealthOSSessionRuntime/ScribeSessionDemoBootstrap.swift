...
public enum ScribeSessionDemoBootstrap {
    public static func makeEnvironment(
        ...
        let healthOSRoot = try resolveHealthOSRoot(fileManager: fileManager)
        let root = healthOSRoot
            .appending(path: "Shared/runtime-data/Users/Shared/HealthOS")
            .standardizedFileURL
    ...
    private static func resolveHealthOSRoot(fileManager: FileManager) throws -> URL {
        var candidate = URL(fileURLWithPath: fileManager.currentDirectoryPath, isDirectory: true)
            .standardizedFileURL

        for _ in 0..<5 {
            if fileManager.fileExists(atPath: candidate.appending(path: "Package.swift").path),
               candidate.lastPathComponent == "HealthOS" {
                return candidate.standardizedFileURL
            }

            let healthOSRoot = candidate.appending(path: "HealthOS")
            if fileManager.fileExists(atPath: healthOSRoot.appending(path: "Package.swift").path) {
                return healthOSRoot.standardizedFileURL
            }
