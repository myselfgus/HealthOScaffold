...
struct Scribe {
    static func main() async {
        guard ProcessInfo.processInfo.arguments.contains("--smoke-test")
            ...
            print("Scribe requires macOS/SwiftUI for interactive mode.")
            return
        }
        ...
        do {
            ...
            print("Scribe smoke test passed (headless fallback).")
            exit(0)
        } catch {
            FileHandle.standardError.write(Data("Scribe smoke test failed: \(error)\n".utf8))
            exit(1)
        }
    }
}
