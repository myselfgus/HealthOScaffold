...
struct VeridiaEntrypoint {
    static func main() async {
        ...
        print("Veridia: patient health identity scaffold placeholder - no final UI shell, no clinical authority (see HealthOS/Shared/docs/architecture/12-veridia.md)")
    }
    ...
    private static func runSmokeTest() async {
        ...
        guard startResult.disposition == .sessionStarted else {
            print("Veridia smoke FAIL: session start returned \(startResult.disposition.rawValue) — \(startResult.issueMessage ?? "no detail")")
            exit(1)
        }
        ...
        guard endResult.disposition == .sessionEnded else {
            print("Veridia smoke FAIL: session end returned \(endResult.disposition.rawValue) — \(endResult.issueMessage ?? "no detail")")
            exit(1)
        }

        print("Veridia scaffold: smoke OK (veridia.session.start + veridia.session.end Boundary verified)")
    }
}
