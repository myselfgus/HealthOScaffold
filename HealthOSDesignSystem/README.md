...
All accents are tuned to read against `.thinMaterial` and Liquid Glass without fighting the system tint. Tint is **always semantic**, never decorative — per `docs/architecture/48`.
...
3. **Production native code** — read `docs/architecture/48-native-macos-ui-design-system-and-app-shells.md` from the source repo as the authoritative spec. Use SwiftUI standard controls before custom glass.
