...
code_references:
  ...
  - path: swift/Sources/HealthOSAACI/AACI.swift
    type: impl
...
testing:
  strategy: Teste arquitetural — verificar que apps/runtimes não publicam types públicos que reimplementam law (consent, gate, habilitation) e que dependem dos contracts do Core. Testes de fronteira em `HealthOSTests`.
...
- **Pressupostos e restrições.** (a) Existe uma constituição única e auditável; (b) compliance regulatório (LGPD, HIPAA-aware, CFM) exige identidade clara de controlador/operador; (c) o Swift Package em [swift/Package.swift](../../swift/Package.swift) já materializa a hierarquia em direção de dependências.
...
Hierarquia constitucional canônica (refletida em [swift/Package.swift](../../swift/Package.swift)):
...
- **Riscos e mitigação.**
  ...
  - **Risco.** Confusão de nomenclatura HealthOScaffold ≠ HealthOS. **Mitigação.** ADR-0012 + glossário em [docs/architecture/17-glossary.md](../architecture/17-glossary.md).
...
- **Testes.** Testes em [swift/Tests/HealthOSTests/](../../swift/Tests/HealthOSTests/) verificam que apps consomem contratos do Core (ex.: `ScribeProfessionalWorkspaceContractsTests`, `VeridiaSessionFacadeTests`).
