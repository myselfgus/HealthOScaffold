...
- **Riscos e mitigação.**
  - **Risco.** Documentação nova introduz "HealthOScaffold ≠ HealthOS" por descuido. **Mitigação.** Glossário canônico em [docs/architecture/17-glossary.md](../architecture/17-glossary.md); revisão de PR.
...
- **Conformidade com Package.swift.** Pacote chama-se `HealthOS` ([swift/Package.swift:5](../../swift/Package.swift:5)) — identidade do sistema preservada no naming técnico.
- **Concurrency.** N/A.
