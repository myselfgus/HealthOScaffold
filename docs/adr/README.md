...
Todas as ADRs seguem o **template canônico** (ver `Template ADR Canônico` em [docs/architecture/](../architecture/) ou no front matter de qualquer ADR existente).
...
- **[TRACEABILITY-MATRIX.md](TRACEABILITY-MATRIX.md)** — matriz ADR ↔ módulos ↔ código ↔ testes ↔ schemas/pipelines.
...
5. Linkar a ADR de docs de arquitetura relevantes (`docs/architecture/`) e do tracker de status (`docs/execution/02-status-and-tracking.md`).
...
- [ ] Verificar que a hierarquia de dependências em [swift/Package.swift](../../swift/Package.swift) **continua respeitada** após a decisão proposta:
