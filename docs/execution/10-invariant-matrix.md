...
| 11. Provenance distingue activation, mediation, gate, finalization | Provenance | Auditoria causal confiável | operações separadas: `gos.activate`, `gos.use.*`, `gate.request`, `gate.resolve`, `document.finalize.soap` | testes de ordenação/proveniência | Não há validador automático de sequência | Validador de trilha de proveniência por sessão |
