...
public actor AACIOrchestrator {
    ...
    public func normalizeTranscript(
        _ request: MentalSpaceNormalizationRequest
    ) async -> MentalSpaceNormalizationResult {
        let routingRequest = ProviderRoutingRequest(
            ...
            finalidade: "mental-space-transcript-normalization",
            allowsRemoteFallback: false,
            fallbackAllowed: true,
            preferLocal: true
        )
        ...
        switch decision {
        case .deniedByPolicy(let reason):
            return MentalSpaceNormalizationResult(
                status: .degraded,
                ...
                issueMessage: "Mental Space normalization denied by provider policy: \(reason.rawValue)."
            )
        case .unavailable(let reason):
            return MentalSpaceNormalizationResult(
                status: .degraded,
                ...
                issueMessage: "Mental Space normalization unavailable: \(reason.rawValue)."
            )
        case .stubOnly(let selection, let reason):
            return MentalSpaceNormalizationResult(
                ...
                issueMessage: "Only a stub local model is available for Mental Space normalization."
            )
        case .selected(let selection), .degradedFallback(let selection, _):
            guard selection.providerKind != .remote else {
                return MentalSpaceNormalizationResult(
                    ...
                    issueMessage: "Remote provider fallback is not allowed for Mental Space normalization."
                )
            }
            guard let provider = await router.languageProvider(for: selection), !selection.isStub else {
                return MentalSpaceNormalizationResult(
                    ...
                    issueMessage: "Selected Mental Space normalization provider was not a real local provider."
                )
            }
            ...
            do {
                let normalized = try await provider.generate(
                    ...
                    context: [
                        "task": "mental-space-transcript-normalization",
                        "promptVersion": "mental-space-normalization-v1",
                ...
                guard !normalizedText.isEmpty else {
                    return MentalSpaceNormalizationResult(
                        ...
                        issueMessage: "Mental Space normalization returned no usable text."
                    )
                }
                return MentalSpaceNormalizationResult(
                    status: .ready,
                    normalizedText: normalizedText,
                    correctionSummary: "Transcript normalized by a local language model; clinician review remains required before use as insight.",
                    providerExecution: ProviderExecutionMetadata(
                        providerId: selection.providerId,
                        providerKind: selection.providerKind.rawValue,
                        taskClass: selection.taskClass.rawValue,
                        ...
                        modelVersion: selection.modelVersion,
                        isStub: false,
                        reason: "local-mental-space-normalization"
                    )
                )
            } catch {
                return MentalSpaceNormalizationResult(
                    ...
                    issueMessage: "Mental Space normalization failed: \(error.localizedDescription)"
                )
            }
        }
    }
