...
The current scaffold uses `GroupBox` + `.thinMaterial` + standard SwiftUI controls. The macOS 26+ Liquid Glass adoption plan per `docs/architecture/48-native-macos-ui-design-system-and-app-shells.md`:
...
- Do not introduce `NavigationSplitView` or sidebar structure until the Scribe screen contracts (`docs/architecture/23-scribe-screen-contracts.md`) require them.
