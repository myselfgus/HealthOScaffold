    ...
    async invoke(request: StewardLLMRequest): Promise<StewardLLMResponse> {
      ...
      if (request.dryRun) {
        return {
          ...base,
          status: 'dryRun',
          text: `[dry-run] anthropic payload prepared${config.enabled ? '' : ' (provider disabled; no network call)'}`,
      ...
      if (config.endpointMode !== 'messages') {
        return {
          ...base,
          status: 'unsupported',
          text: '',
          errorKind: 'unsupported',
          errorMessage: `anthropic adapter supports only messages; got ${config.endpointMode}`,
          durationMs: Date.now() - started,
          outputHash: hashText(''),
        };
      }
      ...
      try {
        ...
        return {
          ...
          errorMessage: `failed to parse anthropic response body: ${safeErrorMessage(parseError)}`,
          durationMs: Date.now() - started,
          outputHash: hashText(''),
        };
      }
      ...
      if (!response.ok) {
        ...
        const providerMessage = extractProviderErrorMessage(raw) ?? `anthropic error ${response.status}`;
        return {
          ...base,
          status: 'providerError',
          text: '',
      ...
      if (!text) {
        return {
          ...
          errorMessage: 'anthropic returned 200 but no extractable assistant text',
          durationMs: Date.now() - started,
          outputHash: hashText(''),
        };
      }
