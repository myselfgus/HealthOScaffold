...
export function createAnthropicProvider(config: StewardLLMProviderConfig): StewardLLMProvider {
  return {
    ...
    async invoke(request: StewardLLMRequest): Promise<StewardLLMResponse> {
      ...
      if (request.dryRun) return { ...base, status: 'dryRun', text: `[dry-run] anthropic payload prepared${config.enabled ? '' : ' (provider disabled; no network call)'}`, raw: { endpointMode: config.endpointMode, enabled: config.enabled }, durationMs: Date.now() - started, outputHash: hashText('[dry-run]') };
      ...
      try {
        ...
        if (!response.ok) {
          ...
          return { ...base, status: 'providerError', text: '', raw, errorKind, errorMessage: `anthropic error ${response.status}`, durationMs: Date.now() - started, outputHash: hashText(JSON.stringify(raw)) };
        }
